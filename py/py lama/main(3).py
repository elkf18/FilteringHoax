...
class Scraper:
    ...
    def get_articles(self, response):
        ...
        for page in range(1, int(self.pages)+1):
            url = f"{self.base_url}?q={self.keywords}#gsc.tab=0&gsc.q=kesehatan&gsc.sort=date&gsc.page={page}"

            page = requests.get(url)
            soup = BeautifulSoup(page.text, "html.parser")

            articles = soup.find_all("div", {"class": "gs-webResult.gs-result"})

            for article in articles:
                title = article.find("div", {"class": "gs-title"}).get_text()
                ...
                published_time = soup2.find("span", {"class": "pull-right"}).get_text()
                ...
                for published_time_WIB in published_time:
                        published_time_WIB = published_time[7:19]
...
if __name__ == '__main__':
    ...
    base_url = f"https://www.suara.com/search"
